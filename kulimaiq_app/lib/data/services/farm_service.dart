...
class FarmService {
  ...
  Future<Farm> createFarm(Farm farm) async {
    ...
    final db = await _databaseService.database;
    await db.insert('farms', _toRow(toSave),
  ...
  Future<void> deleteFarm(String id) async {
    ...
    // Fire-and-forget backend delete (errors are silently ignored).
    try {
      _backendApiService.deleteFarm(id);
    } catch (_) {}
  }
  ...
  Future<double?> _computeHealthScore(Database db, String farmId) async {
    ...
    return (total / rows.length).clamp(0, 100);
  }
  ...
  Farm _fromRow(Map<String, Object?> row, double? healthScore) {
    ...
    return Farm(
      ...
      sizeHa: (row['size_ha'] as num).toDouble(),
      crops: crops,
